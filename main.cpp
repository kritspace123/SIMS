...
public:
    Simulator(int width, int height, int num_wolfs, int num_rabbits, int** animals);
...
///Simulator
Simulator::Simulator(int width, int height, int num_wolfs, int num_rabbits, int** animals): m_Width(width), m_Height(height), m_CouAnimals(num_wolfs+num_rabbits), m_Animal(
        nullptr) {
...
void Simulator::ProcessDying() {
    for(int i = 0; i < m_CouAnimals; ++i){
        if (m_Animal[i] != nullptr) {
            if ((m_Animal[i]->IsRabbit() && m_Animal[i]->GetAge() == 10)|| ((!m_Animal[i]->IsRabbit()) && m_Animal[i]->GetAge() == 15)) {
                int j = i;
